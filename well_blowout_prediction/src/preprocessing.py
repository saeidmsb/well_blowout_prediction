...
def preprocess_data(input_path, output_path, scaler_path):
    ...
    os.makedirs(os.path.dirname(scaler_path), exist_ok=True)
    scaler_bundle = {"scaler": scaler, "columns": columns_to_scale}
    joblib.dump(scaler_bundle, scaler_path)

    print(f" Data processed and saved to '{output_path}'.")
    print(f" Scaler and columns saved to '{scaler_path}'.")
...
def create_sequences(data, sequence_length=50):
    ...
    for i in range(len(data_array) - sequence_length):
        ...
        labels.append(label_array[i + sequence_length])  
